...
TEST (TIME, REAL_TIME)
{
  ...
  g_message("Current local time: %s at %s\n",
      isolocaltime,
      g_date_time_get_timezone_abbreviation(current_localdatetime));
...
TEST (TIME, GDateTime)
{
  ...
  const gchar *tz_abbr
      = g_date_time_get_timezone_abbreviation (current_datetime);
...
TEST (DATETIME, TimeZone)
{
  ...
  gint tz_interval = g_time_zone_find_interval (
      timezone, G_TIME_TYPE_UNIVERSAL, current_utc);
  ...
  g_message ("Timezone at %s %s with offset %f hours, %s\n", tz_id, tz_abbr,
             tz_offset / 3600.0, is_dst ? "Day Light Saving" : "non-dst");
